...
- **Dynamic-catalog providers (OpenRouter)** — the model list is fetched **live** from the provider's own `/models` endpoint (using the provider's base URL and API key) instead of the bundled catalog, so newly published models appear without waiting for an Axiom release. The live list replaces the curated one; if the live fetch fails, Axiom falls back to the bundled `@earendil-works/pi-ai` catalog so the picker still works offline.
