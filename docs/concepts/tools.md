...
| Tool | Notes |
|---|---|
| `provider_quota` | Returns the current subscriber usage quota — or, for credit-based gateways such as Radius, the prepaid credit balance — for LLM providers (Anthropic Claude Pro/Max, ChatGPT Codex, OpenCode, z.ai, Radius, …). Accepts an optional `providerId` to scope to one provider and an optional `refresh` flag to force a live fetch (for the selected provider, or all of them when `providerId` is omitted). Without `refresh`, returns the cached snapshot from the [subscriber usage quota](../web-ui/providers#subscriber-usage-quota) monitor (polled in the background). Like the provider API, the tool answers **admin users only**; a provider refreshed less than a minute ago keeps serving cached data. |
