...
A **task** is a self-contained agent run that proceeds in the background. The user (or the agent itself) hands off a prompt; a fresh `PiAgent` instance spins up in isolation, works through the prompt with the full tool and skill registry, and reports back when it's done.
...
- **`running`** — the agent is actively working. The `TaskRunner` keeps the live `PiAgent` instance in memory in a `runningTasks` map.
...
Every task runs in its own `PiAgent` with its own session ID — separate from the parent chat. A task agent has:
